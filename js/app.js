    ...
    try {
        ...
        try {
            if (typeof DOMElements !== 'undefined' && DOMElements) {
                setupEventListeners?.();
                console.log('[app] Event listeners bound after data load.');
            } else {
                console.warn('[app] DOMElements not ready after loadData, waiting...');
                const waitForDOM = setInterval(() => {
                    if (typeof DOMElements !== 'undefined' && DOMElements) {
                        clearInterval(waitForDOM);
                        setupEventListeners?.();
                        console.log('[app] Event listeners bound after waiting.');
                    }
                }, 200);
            }
        } catch(e) {
