...
if (msg.revoked) {
    ...
    // 若发送者是用户则显示“你撤回了一条消息”，否则显示“对方撤回了一条消息”
    const who = (msg.sender === 'user') ? '你' : (settings.partnerName || '对方');
    revokeDiv.textContent = `${who}撤回了一条消息`;
    fragment.appendChild(revokeDiv);
    return fragment;
}
...
if (msg.voice && msg.voice.url) {
    ...
    content += `
        <div class="voice-message" data-url="${msg.voice.url}" data-duration="${dur}">
            <i class="fas fa-play-circle"></i>
            <span class="voice-duration">${dur}"</span>
            ${fakeText ? `<div class="voice-transcript">${fakeText}</div>` : ''}
        </div>
    `;
}
    if (isImageOnly) {
...
// 仅当是自己发送的、未撤回
if (msg.sender === 'user' && !msg.revoked && (Date.now() - new Date(msg.timestamp)) {
    actionsHTML += `<button class="meta-action-btn revoke-btn" title="撤回"><i class="fas fa-undo-alt"></i></button>`;
}
...
function revokeMessage(msgId) {
    ...
    msg.voice = null;
    if (msg.replyTo) {
        msg.replyTo.text = '[消息已撤回]';
    }
    throttledSaveData();
    renderMessages(true);
    showNotification('已撤回一条消息', 'success');
}
                    ...
                    if (shouldSendSticker) {
                        ...
                        setTimeout(() => {
                            addMessage({
                                ...
                                type: 'normal'
                                revoked: false,          // 是否被撤回
                                canRevoke: true          // 可由发送时间动态判断，但可省略
...
document.addEventListener('click', function(e) {
    ...
    if (!revokeBtn) return;
    const wrapper = revokeBtn.closest('.message-wrapper');
    if (!wrapper) return;
