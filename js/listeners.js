...
const initMusicPlayer = async () => {
    ...
    const latestSystemSongs = [
        // ...（原有歌曲数组，为避免过长省略，实际你保留原内容）
    ];
    // ...（原有逻辑全部保留，因为里面没有使用 DOMElements，只用了 getElementById）
    // 但由于篇幅，我会保持原函数不变，但注意确保所有用到的 DOM 元素存在。
    // 实际你可以直接复制原函数内容过来。
    // 我这里简写示意，实际你保留原有代码。
    console.warn('initMusicPlayer 需要保留原代码，此处为占位。');
};
