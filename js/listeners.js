function setupEventListeners() {
    if (!DOMElements) {
        console.warn('[listeners] DOMElements not ready, retrying...');
        setTimeout(setupEventListeners, 200);
        return;
    }
    console.log('[listeners] Starting event bindings...');
    
    // 依次执行，每个独立捕获错误
    const tasks = [
        { name: 'initCoreListeners', fn: initCoreListeners },
        { name: 'initModalListeners', fn: initModalListeners },
        { name: 'initChatActionListeners', fn: initChatActionListeners },
        { name: 'initHeaderAndSettingsListeners', fn: initHeaderAndSettingsListeners },
        { name: 'initDataManagementListeners', fn: initDataManagementListeners },
        { name: 'initNewFeatureListeners', fn: initNewFeatureListeners },
        { name: 'setupTutorialListeners', fn: setupTutorialListeners },
        { name: 'initMoodListeners', fn: initMoodListeners },
        { name: 'initDecisionModule', fn: initDecisionModule },
        { name: 'initAnniversaryModule', fn: initAnniversaryModule },
        { name: 'initThemeEditor', fn: initThemeEditor },
        { name: 'initThemeSchemes', fn: initThemeSchemes },
        { name: 'initComboMenu', fn: initComboMenu }
    ];
    ...
    tasks.forEach(task => {
        try {
            if (typeof task.fn === 'function') {
                task.fn();
                console.log(`[listeners] ✓ ${task.name} executed`);
            } else {
                console.warn(`[listeners] ⚠️ ${task.name} not a function, skipping`);
            }
        } catch (e) {
            console.error(`[listeners] ✗ ${task.name} failed:`, e);
        }
    });
    console.log('[listeners] All event bindings attempted.');
}

function initChatActionListeners() {
    if (!DOMElements) return;
    const { chatContainer, batchPreview, batchBtn, messageInput } = DOMElements;
    ...
    if (chatContainer) {
        chatContainer.addEventListener('click', (e) => {
            ...
            if (target.classList.contains('reply-btn')) {
                ...
                const targetMessageElement = chatContainer.querySelector(`[data-id="${message.id}"]`);
    ...
    if (batchPreview) {
        batchPreview.addEventListener('click', (e) => {
            ...
            if (e.target.matches('.batch-cancel-btn')) {
                ...
                const placeholder = "";
                if (messageInput) messageInput.placeholder = placeholder.length > 20 ? placeholder.substring(0, 20) + "...": placeholder;
                batchMessages = [];
            }
        });
    }
...
function initModalListeners() {
    if (!DOMElements) return;
    const { editModal, pokeModal, cancelCoinResult, coinTossOverlay, sendCoinResult, chatModal, dataModal } = DOMElements;
    ...
    if (closeDataBtn && dataModal) {
        closeDataBtn.addEventListener('click', () => {
            hideModal(dataModal.modal);
        });
    }

    if (editModal && editModal.input && editModal.save) {
        editModal.input.addEventListener('input', () => {
            editModal.save.disabled = !editModal.input.value.trim();
        });
    }

    if (pokeModal && pokeModal.save && pokeModal.input) {
        pokeModal.save.addEventListener('click', () => {
            const verb = pokeModal.input.value.trim() || settings.myPokeText || '拍了拍';
            ...
            hideModal(pokeModal.modal);
            pokeModal.input.value = settings.myPokeText || '';
    ...
    if (cancelCoinResult && coinTossOverlay) {
        cancelCoinResult.addEventListener('click', () => {
            coinTossOverlay.classList.remove('visible', 'finished');
            lastCoinResult = null;
        });
    }

    if (sendCoinResult && coinTossOverlay) {
        sendCoinResult.addEventListener('click', () => {
            if (lastCoinResult) {
...
function initHeaderAndSettingsListeners() {
    if (!DOMElements) return;
    const { partner, me, settingsModal, favoritesModal, appearanceModal, avatarModal, editModal } = DOMElements;
    ...
    const openNameModal = (isPartner) => {
        ...
        modal.save.onclick = () => {
            ...
            if (newName) {
                if (isPartner) settings.partnerName = newName;
                else settings.myName = newName;
                throttledSaveData();
                updateUI();
    ...
    const openAvatarModal = (isPartner) => {
        ...
        if (uploadBtn) {
            uploadBtn.addEventListener('click', () => {
                ...
                if (pasteUrlBtn) pasteUrlBtn.classList.remove('active');
            });
        }
            ...
            fileInput.addEventListener('change', function(e) {
                ...
                if (file) {
                    ...
                    cropImageToSquare(file, 300).then(base64Data => {
                        ...
                        if (previewImg) previewImg.src = currentAvatarData;
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', () => {
                if (currentAvatarData) {
                    const targetAvatar = isPartner ? partner.avatar : me.avatar;
                    updateAvatar(targetAvatar, currentAvatarData);
                    throttledSaveData();
    ...
    if (partner && partner.name) partner.name.addEventListener('click', () => openNameModal(true));
    if (me && me.name) me.name.addEventListener('click', () => openNameModal(false));
    if (partner && partner.avatar) partner.avatar.addEventListener('click', () => openAvatarModal(true));
    if (me && me.avatar) me.avatar.addEventListener('click', () => openAvatarModal(false));

    if (me && me.statusContainer && me.statusText) {
        me.statusContainer.addEventListener('click', () => {
            const statusTextElement = me.statusText;
            const statusContainer = me.statusContainer;
            if (statusContainer.querySelector('input')) return;
            const input = document.createElement('input');
            input.type = 'text';
            input.id = 'my-status-input';
            input.value = statusTextElement.textContent;
            const saveStatus = () => {
    ...
    if (settingsModal && settingsModal.settingsBtn) {
        settingsModal.settingsBtn.addEventListener('click', () => {
            showModal(settingsModal.modal);
        });
    }

    if (favoritesModal && favoritesModal.favoritesBtn) {
        favoritesModal.favoritesBtn.addEventListener('click', () => {
    ...
    if (_chatSettingsEl && settingsModal) {
        _chatSettingsEl.addEventListener('click', () => {
            ...
            if (DOMElements.chatModal) showModal(DOMElements.chatModal.modal);
            setupAvatarFrameSettings();
        });
    }

    const _advancedEl = document.getElementById('advanced-settings');
    if (_advancedEl && settingsModal) {
        _advancedEl.addEventListener('click', () => {
            hideModal(settingsModal.modal);
            if (DOMElements.advancedModal) showModal(DOMElements.advancedModal.modal);
        });
    }

    const _dataSettingsEl = document.getElementById('data-settings');
    if (_dataSettingsEl && settingsModal) {
        _dataSettingsEl.addEventListener('click', () => {
            hideModal(settingsModal.modal);
            if (DOMElements.dataModal) showModal(DOMElements.dataModal.modal);
        });
    }
    ...
    function updateAvatarPreview(shape, cornerRadius) {
        ...
        const partnerImg = (partner && partner.avatar) ? partner.avatar.querySelector('img') : null;
        const myImg = (me && me.avatar) ? me.avatar.querySelector('img') : null;
    ...
    document.querySelectorAll('.avatar-shape-btn-2').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (typeof applyAvatarShapeToDOM === 'function') {
                applyAvatarShapeToDOM('my', shape);
                applyAvatarShapeToDOM('partner', shape);
            }
    ...
    function updateDelayUI() {
        if (minDelaySlider && minDelayValue && maxDelaySlider && maxDelayValue) {
    ...
    if (_appearanceEl && settingsModal && appearanceModal) {
        _appearanceEl.addEventListener('click', () => {
            hideModal(settingsModal.modal);
            if (typeof window.hideAppearancePanel === 'function') window.hideAppearancePanel();
            ...
            showModal(appearanceModal.modal);
            setTimeout(() => { 
                if (typeof updateAvatarSettingsUI === 'function') updateAvatarSettingsUI(); 
                if (typeof setupAppearancePanelFrameSettings === 'function') setupAppearancePanelFrameSettings();
            }, 100);
        });
    }
    if (appearanceModal && appearanceModal.closeBtn) {
        appearanceModal.closeBtn.addEventListener('click', () => {
            hideModal(appearanceModal.modal);
        });
    }
    ...
    const updateAutoSendUI = () => {
        ...
        if (autoSendControl) autoSendControl.style.display = settings.autoSendEnabled ? "flex" : "none";
        const currentVal = settings.autoSendInterval || 5;
        if (autoSendSlider) autoSendSlider.value = currentVal;
        if (autoSendValue) autoSendValue.textContent = `${currentVal}分钟`;
    };
    updateAutoSendUI();
    ...
    if (autoSendSlider) {
        ...
        if (autoSendValue) autoSendValue.textContent = `${settings.autoSendInterval || 5}分钟`;
        autoSendSlider.addEventListener('input', (e) => {
            const val = parseInt(e.target.value);
            settings.autoSendInterval = val;
            if (autoSendValue) autoSendValue.textContent = `${val}分钟`;
        });
...
// 朋友圈入口按钮
const momentsEntry = document.getElementById('moments-entry-btn');
if (momentsEntry) {
    momentsEntry.addEventListener('click', () => {
        if (typeof MomentsApp !== 'undefined') {
            ...
            if (typeof showNotification === 'function') {
                showNotification('朋友圈功能未加载，请刷新页面', 'error', 2000);
            }
        }
    });
}
...
function initNewFeatureListeners() {
    ...
    if (_batchFavEl) {
        _batchFavEl.addEventListener('click', () => {
            if (DOMElements.favoritesModal) hideModal(DOMElements.favoritesModal.modal);
    ...
    if (DOMElements.anniversaryAnimation && DOMElements.anniversaryAnimation.closeBtn) {
    ...
    if (_statsFuncEl) {
        _statsFuncEl.addEventListener('click', () => {
            if (advancedModal) hideModal(advancedModal.modal);
            renderStatsContent();
            if (DOMElements.statsModal) showModal(DOMElements.statsModal.modal);
        });
    }
    ...
    if (musicToggle) {
        musicToggle.addEventListener('click', () => {
            ...
            if (settings.musicPlayerEnabled) {
                ...
                if (playlistEl) playlistEl.classList.remove('active');
...
function initDataManagementListeners() {
    ...
    if (creditsBtn) {
        creditsBtn.addEventListener('click', () => {
            if (dataModal) hideModal(dataModal.modal);
            const disclaimerModal = document.getElementById('disclaimer-modal');
            if (disclaimerModal) showModal(disclaimerModal);
        });
    }

    const sessionModal = DOMElements.sessionModal;
    if (sessionModal && sessionModal.managerBtn) {
        sessionModal.managerBtn.addEventListener('click', () => {
            renderSessionList();
            showModal(sessionModal.modal);
        });
    }
    if (sessionModal && sessionModal.createBtn) {
        sessionModal.createBtn.addEventListener('click', async () => {
            await createNewSession(false);
            renderSessionList();
            showNotification('新会话已创建', 'success');
        });
    }
    if (sessionModal && sessionModal.list) {
...
const initMusicPlayer = async () => {
    // 请保留你原来的完整 initMusicPlayer 实现，此处仅为占位
    console.warn('initMusicPlayer 需要保留原代码，请确保此函数完整。');
};

function initCoreListeners() {
    if (!DOMElements) return;
    const { chatContainer, sendBtn, messageInput, attachmentBtn, imageInput, continueBtn, batchBtn } = DOMElements;
    ...
    if (sendBtn) {
        sendBtn.addEventListener('click', () => isBatchMode ? addToBatch() : sendMessage());
    }
    if (messageInput) {
    ...
    if (attachmentBtn) {
        attachmentBtn.addEventListener('click', () => {
            ...
            const fileInputEl = document.getElementById('image-file-input');
            const urlInputEl = document.getElementById('image-url-input');
            ...
            function switchUploadMode(isFileMode) {
                ...
                if (isFileMode) {
                    uploadBtn.classList.add('active');
                    if (fileInputEl) fileInputEl.style.display = 'block';
                    if (urlInputEl) urlInputEl.style.display = 'none';
                } else {
                    pasteUrlBtn.classList.add('active');
                    if (fileInputEl) fileInputEl.style.display = 'none';
                    if (urlInputEl) urlInputEl.style.display = 'block';
                    if (urlInputEl) urlInputEl.focus();
                }
                if (previewDiv) previewDiv.style.display = 'none';
                if (sendBtn) sendBtn.disabled = true;
                currentImageData = null;
            }
            if (uploadBtn) uploadBtn.addEventListener('click', () => switchUploadMode(true));
            if (pasteUrlBtn) pasteUrlBtn.addEventListener('click', () => switchUploadMode(false));
    ...
    if (continueBtn) continueBtn.addEventListener('click', simulateReply);
